...
import { unblockedEmail, forgotPasswordQuestions } from "../API";
import ReCAPTCHA from "react-google-recaptcha";
const UnblockedUser = () => {
	const [user, setUser] = useState({ user: "", option: 0, recoveryUser:true });
		...
		try {
			...
			if (user.option === 2) {
				...
				if (res.status >= 400) {
					...
					navigate("/unblocked/user/questions", { state:{questions:res.data,user:user.user,recovery:'user'} });
				}
			}
		} catch (e) {
			setIsSubmit(false);
		...
		<>
			{
				...
				>
					{" "}
					<h1>Correo de desbloqueo de usuario enviado!</h1>{" "}
					<p>
						Hemos enviado un link en el que podras desbloquear el usuario! ve y revisa tu correo por favor, tienes 15
			...
			>
				<h1 className="title">Desbloqueo de usuario</h1>
				<p className="text">
					Para recuperar el acceso sigue los pasos a continuacion
				</p>
						...
						>
							...
							<div className="content">
								...
								<p>
									...
									intentar desbloquear su usuario por este
									metodo
								</p>
							</div>
						</div>

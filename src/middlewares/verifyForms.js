...
export const verifyCreate = async (data, isUser) => {
    ...
    if (contact) {
        ...
        //Validation
        if (invalidsEmails.length > 0)
            return { message: "Email/s de estudiante invalido" };
        if (invalidsPhones.length > 0)
            return { message: "Telefono/s de estudiante invalido" };

        if (contact.address_1 && contact.address_1.length > 150)
            return {
                message: "La direccion debe de tener maximo 150 caracteres!",
            };
        if (contact.address_2 && contact.address_2.length > 150)
            return {
                message: "La direccion debe de tener maximo 150 caracteres!",
            };
    }
    ...
    if (!isUser) {
        ...
        if (studentFind)
            return {
                message:
                    "El estudiante ha sido registrado anteriormente en el sistema!",
            };
    }else{
...
export const verifyXls = async (data, rows, studentsRegister) => {
    ...
    if (!/^[A-Za-záéíóúñ'´ ]+$/.test(firstname)) {
        return {
            message: `El nombre del estudiante en la fila: ${rows} contiene caracteres invalidos`,
        };
    }
    ...
    if (studentFind) {
        return {
            message: `El estudiante de la fila: ${rows} ha sido registrado anteriormente`,
        };
    }
...
export const verifyRep = async (data, id) => {
    ...
    if (contact) {
        ...
        //Validation
        if (invalidsEmails.length > 0)
            return { message: "Email/s de representante invalido" };
        if (invalidsPhones.length > 0)
            return { message: "Telefono/s de representante invalido" };
        if (contact.address_1 && contact.address_1.length > 150)
            return {
                message: "La direccion debe de tener maximo 150 caracteres!",
            };
    ...
    if (repFound)
        return { message: "El representante tiene una cedula ya registrada" };
    return true;
};
...
export const list = async (req, res) => {
    try {
        ...
        if (students.totalDocs < 0) {
            return res
                .status(404)
                .json({ message: "Estudiantes no encontrados" });
        }
...
export const saveScore = async (req, res) => {
    try {
        ...
        if (studentFind.subjects.length !== scores.length)
            return res.status(400).json({
                message: "No se pudo procesar las notas del estudiante",
            });
...
export const createStudent = async (req, res) => {
    try {
        const { ci, firstname, lastname, contact, rep_data, section_id } =
            req.body;
...
export const createStudents = async (req, res) => {
    try {
        ...
        if (/[^.]+$/.exec(req.file.filename) == "csv") {
            fs.createReadStream(archive)
                ...
                .on("data", async (row) => {
                    ...
                    if (studentsRegister.find((el) => el === ci)) {
                        return rowErrors.push(
                    ...
                    if (sectionFound) {
                        await addStudentsSectionRegistered(sectionFound.id, [
                            newStudent._id,
                        ]);
                    }
                })
                ...
                .on("end", async () => {
                    ...
                    return res.json({
                        message:
                            "Todos los estudiantes del archivo CSV añadidos",
                    });
                });
        }
        //Check file xlsx
        if (/[^.]+$/.exec(req.file.filename) == "xlsx") {
            ...
            return res.json({
                message:
                    "Todos los estudiantes del archivo han sido registrados",
            });
        }
    } catch (err) {
        console.log(err);
        ...
        await student.updateOne(
            { _id: req.params.id },
            {
                $set: {
                    ...
                    graduate:
                        studentInfo.graduate || studentInfoActual.graduate,
                    status: req.body.status || false,
                    contact: req.body.contact || studentInfoActual.contact,
                    representative:
                        studentInfo.rep_data && studentInfo.rep_data._id,
                    last_modify: dateFormat(
                        now,
                        "dddd, d De mmmm , yyyy, h:MM:ss TT"
                    ),
                },
                $unset: studentInfo.deleteSection
                    ? { section: 1 }
                    : {} && studentInfo.rep_data.id
                    ? { representative: 1 }
                    : {},
            },
            { upsert: true }
...
export const deleteStudents = async (req, res) => {
    try {
        ...
        for (const id of ids) {
            ...
            if (studentFound) {
                const chest_student = chest.findOne({
